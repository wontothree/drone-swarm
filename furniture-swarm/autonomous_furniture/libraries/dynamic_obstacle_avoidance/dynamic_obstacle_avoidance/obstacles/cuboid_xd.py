...
class CuboidXd(obstacles.Obstacle):
    ...
    def get_normal_direction_with_distance(
        self, position, in_global_frame: bool = True
    ):
        ...
        if not any(ind_relevant):
            ...
            return np.ones(position.shape), 0
    ...
    def get_distance_to_surface(
        self, position, in_global_frame: bool = True, margin_absolut: float = None
    ):
        if in_global_frame:
            position = self.pose.transform_position_to_relative(position)
    ...
    def get_gamma(
        ...
        is_boundary=None,
        boundary_power_factor = 1, #: int | float = 1,
    ) -> float:
