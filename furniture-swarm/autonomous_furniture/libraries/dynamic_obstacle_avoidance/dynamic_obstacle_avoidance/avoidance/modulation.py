...
def compute_diagonal_matrix(
    ...
    if Gamma <= 1 and treat_obstacle_special:
        ...
        delta_eigenvalue = 1.0 / (abs(Gamma) ** (1 / repulsion_coeff))
...
def compute_decomposition_matrix(obs, x_t, in_global_frame=False, dot_margin=0.02):
    ...
    normal_vector, distance = obs.get_normal_direction_with_distance(x_t, in_global_frame=in_global_frame)
...
def obs_avoidance_interpolation_moving(
    ...
    for n in range(N_obs):
        ...
        if gamma_value < cut_off_gamma:
            Gamma.append(gamma_value)
            obs_id_cut_off.append(n)
            obs_cut_off.append(obs[n])
    ...
    relative_velocity_hat = np.zeros((dim, N_obs_cut_off))
    relative_velocity_hat_magnitude = np.zeros((N_obs_cut_off))

    n = 0
    for n in range(N_obs_cut_off):
        idx_obs = obs_id_cut_off[n]
        if obs[idx_obs].repulsion_coeff > 1 and E_orth[:, 0, n].T.dot(relative_velocity) < 0:
            ...
            if not evaluate_in_global_frame:
                relative_velocity_temp = obs[idx_obs].transform_global2relative_dir(
                    relative_velocity
                )
            else:
            ...
            if obs[idx_obs].repulsion_coeff < 0:
            ...
            elif not obs[idx_obs].tail_effect and (
                (relative_velocity_trafo[0] > 0 and not obs[idx_obs].is_boundary)
                or (relative_velocity_trafo[0] < 0 and obs[idx_obs].is_boundary)
            ):
            ...
            if not evaluate_in_global_frame:
                relative_velocity_hat[:, n] = obs[idx_obs].transform_relative2global_dir(
                    relative_velocity_hat[:, n]
                )
        ...
        if repulsive_obstacle:
            ...
            if Gamma[n] < (1 + repulsive_gammaMargin):
                ...
                if not obs[idx_obs].is_boundary:
                    repulsive_speed *= -1

                pos_rel = obs[idx_obs].get_reference_direction(position, in_global_frame=True)
